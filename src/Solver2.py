...
ALNS_MIN_REMOVE = 3
ALNS_MAX_REMOVE = 6
ALNS_REPAIR_CANDIDATE_LIMIT = 55

TABU_TENURE = 10
MAX_TABU_ITERATIONS = 80
...
def repair_solution(model, routes, time_weight, capacity_weight):
    ...
    while True:
        ...
        candidate_nodes = unvisited[:ALNS_REPAIR_CANDIDATE_LIMIT]
...
def alns_search(
    ...
    for _ in range(ALNS_ITERATIONS):
        ...
        candidate_routes = improve_by_extra_insertions(model, candidate_routes)
...
def solve(model, solution_file, enforce_mandatory=True):
    print("Running Weighted Greedy + ALNS + Tabu...")
    ...
    print(
        f"Best params: seed={best_params[0]}, "
        f"TW={best_params[1]}, CW={best_params[2]}, RCL={best_params[3]}"
    )
